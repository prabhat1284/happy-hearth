...
function indexModel()
{
    ...
    this.userlogin=(collection_name,userDetails)=>{
        return new Promise((resolve,reject)=>{
            db.collection(collection_name).find({"username":userDetails.username,"password":userDetails.password,"status":1}).toArray((err,result)=>{
                err  ? reject(err) : resolve(result);        
            })
        })
    }

    this.contact=(collection_name,userDetails)=>{
        return new Promise((resolve,reject)=>{
            db.collection(collection_name).find().toArray((err,result)=>{
                if(err)
                    reject(err);
